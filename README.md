...
- **OpenAI integration with tool calling** (`src/llm/openai.js`, `src/tools/functions.js`) — explicit JSON schemas for `get_order_status`, `search_products`, `escalate_to_human` (no free-form JSON guessing). Uses the `openai` SDK.
...
- **Degraded-mode fallback** — if `OPENAI_API_KEY` is missing or the LLM call fails, the bot replies with the static fallback message from the spec instead of crashing.
...
src/
  ...
  llm/openai.js            OpenAI chat + tool-calling loop
